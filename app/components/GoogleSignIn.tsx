...
export const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ className = '' }) => {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`flex items-center gap-3 px-6 py-3 bg-white text-gray-800 rounded-lg font-medium hover:bg-gray-100 transition-all shadow-md hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed ${className}`}
    >
      {isLoading ? (
        <>
